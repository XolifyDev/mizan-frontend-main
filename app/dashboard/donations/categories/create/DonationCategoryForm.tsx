...
type Props = {
  mode: 'create' | 'edit';
  initialValues?: Partial<DonationCategoryFormValues>;
  onSubmit: (data: DonationCategoryFormValues) => Promise<void>;
  onCancel: () => void;
...
export default function DonationCategoryForm({ mode, initialValues, onSubmit, onCancel, isSubmitting }: Props) {
  const { control, handleSubmit, setValue, watch, reset } = useForm<DonationCategoryFormValues>({
    defaultValues: {
      ...
      headerBgColor: initialValues?.headerBgColor || "#550C18",
      showOnKiosk: initialValues?.showOnKiosk ?? true,
      excludeFromReceipts: initialValues?.excludeFromReceipts || false,
      allowPledge: initialValues?.allowPledge || false,
      quickDonate: initialValues?.quickDonate || false,
      hideTitle: initialValues?.hideTitle || false,
      defaultAmounts: initialValues?.defaultAmounts || "10, 25, 50, 100",
      min: initialValues?.min || 1,
      max: initialValues?.max || 10000,
      enforceMax: initialValues?.enforceMax || false,
      allowCustomAmount: initialValues?.allowCustomAmount ?? true,
      intervals: initialValues?.intervals || ["onetime"],
      defaultInterval: initialValues?.defaultInterval || "onetime",
      recurringCountOptions: initialValues?.recurringCountOptions || [],
      recurring: initialValues?.recurring || false,
      enableAppleGooglePay: initialValues?.enableAppleGooglePay || false,
      coverFee: initialValues?.coverFee || false,
      coverFeeDefault: initialValues?.coverFeeDefault || false,
      allowAnonymous: initialValues?.allowAnonymous || false,
      allowComments: initialValues?.allowComments || false,
      collectAddress: initialValues?.collectAddress || false,
      collectPhone: initialValues?.collectPhone || false,
      mailingListOptIn: initialValues?.mailingListOptIn || false,
      goalAmount: initialValues?.goalAmount || 0,
      ctaMessage: initialValues?.ctaMessage || "",
      designations: initialValues?.designations || [],
      amountsPerInterval: initialValues?.amountsPerInterval || {},
      customLabel: initialValues?.customLabel || "",
      complianceText: initialValues?.complianceText || "",
      appreciation: initialValues?.appreciation || "",
  ...
  const handleLogoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setLogoFile(file);
      setValue("logo", URL.createObjectURL(file)); // for preview only
    }
  };
  ...
  const handleFeaturedImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setValue("featuredImage", URL.createObjectURL(file)); // for preview only
    }
  };
  ...
  const uploadImage = async (file: File) => {
    ...
    toast({ title: "Image Uploaded", description: "Image uploaded successfully!", variant: "default" });
    return data.url || data.fileUrl || data.imageUrl; // adjust if your API returns a different field
  };

  const internalOnSubmit = async (data: DonationCategoryFormValues) => {
    try {
    ...
    <form onSubmit={handleSubmit(internalOnSubmit)} className="space-y-6">
      ...
      <Card className="bg-white border-[#550C18]/10">
        ...
        <CardContent className="space-y-6">
          {/* Main grid: Color pickers + image uploaders */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            ...
            {/* Logo Upload */}
            <div className="flex flex-col items-center border rounded-lg p-4 bg-gray-50 min-h-[140px]">
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <Button className="bg-[#550C18] hover:bg-[#78001A] text-white" type="submit" disabled={isSubmitting}>
          {isSubmitting ? (mode === "edit" ? "Saving..." : "Creating...") : (mode === "edit" ? "Save Changes" : "Create Category")}
        </Button>
      </div>
    </form>
  );
