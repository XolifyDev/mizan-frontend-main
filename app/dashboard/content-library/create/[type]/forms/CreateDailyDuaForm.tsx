...
const schema = z.object({
  ...
  dua: z.string().min(2, "Dua is required"),
});

type FormValues = z.infer<typeof schema>;

export default function CreateDailyDuaForm() {
  const form = useForm<FormValues>({
    resolver: zodResolver(schema),
    defaultValues: { title: "", description: "", dua: "" },
  });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="max-w-xl mx-auto p-8 bg-white rounded-lg shadow-md space-y-4">
        <h2 className="text-2xl font-bold mb-4">Add Daily Dua</h2>
        <FormField
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Dua</FormLabel>
              <FormControl>
                <Textarea placeholder="Dua content" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
