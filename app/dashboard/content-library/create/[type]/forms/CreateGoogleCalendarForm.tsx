...
const schema = z.object({
  ...
  calendarUrl: z.string().url("Please enter a valid Google Calendar URL"),
...
export default function CreateGoogleCalendarForm() {
  ...
  const onSubmit = async (values: FormValues) => {
    await createContentWithConfig({
      ...
      data: { calendarUrl: values.calendarUrl },
    });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="max-w-xl mx-auto p-8 bg-white rounded-lg shadow-md space-y-4">
        <h2 className="text-2xl font-bold mb-4">Add Google Calendar Event</h2>
        <FormField
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder="Google Calendar URL" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
