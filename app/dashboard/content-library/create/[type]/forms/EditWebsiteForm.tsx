...
export default function EditWebsiteForm({ id }: { id: string }) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast({ title: "Error", description: err.message || "Failed to update website content", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto p-8 bg-white rounded-lg shadow-md space-y-4">
        <div className="flex flex-row w-full items-center justify-between mb-4">
          <h2 className="text-2xl font-bold">Edit Website</h2>
        ...
        <div className="flex flex-row w-full">
          <Button size={"lg"} type="submit" className="ml-auto text-md" disabled={loading}>{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save Changes"}</Button>
        </div>
      </form>
    </Form>
  );
