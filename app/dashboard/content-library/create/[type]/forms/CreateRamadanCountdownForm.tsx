...
export default function CreateRamadanCountdownForm() {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    toast({
      title: "Ramadan Countdown Created",
      description: "Ramadan Countdown has been created successfully",
    });
  ...
  useEffect(() => {
    const dayssTillRamadan = Math.floor((new Date(watchEndDate) - new Date(watchStartDate)) / (1000 * 60 * 60 * 24));
    setDayssTillRamadan(dayssTillRamadan);
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="mx-auto p-8 bg-white rounded-lg shadow-md space-y-4">
        <div className="flex flex-row w-full items-center justify-between mb-4">
          <h2 className="text-2xl font-bold">Add Ramadan Countdown</h2>
